...
function SearchDropdown({
  ...
  return (
    <div ref={ref} className="relative">
      <div className="flex items-center gap-1 bg-[var(--color-cream)] border border-[var(--color-border)] rounded-lg px-2.5 py-1.5 focus-within:border-[var(--color-border-dark)]">
      ...
      {open && filtered.length > 0 && (
        <div className="absolute top-full mt-1 left-0 right-0 bg-[var(--color-cream)] border border-[var(--color-border)] rounded-lg shadow-[var(--shadow-medium)] z-50 overflow-hidden">
          {filtered.map(t => {
    ...
    <div className="space-y-3 animate-fade-in">
      ...
      <div className="flex flex-wrap items-center gap-2">
        ...
        <div ref={presetsRef} className="relative">
          ...
          {showAddPreset && (
            <div className="absolute top-full mt-1 left-0 bg-[var(--color-cream)] border border-[var(--color-border)] rounded-lg shadow-[var(--shadow-medium)] z-50 overflow-hidden min-w-[160px]">
              {PRESETS.map(p => (
                <button
                  key={p.label}
...
function WorkHoursPanel({
  ...
  return (
    <div
      ref={ref}
      className="absolute top-full mt-1 left-0 bg-[var(--color-cream)] border border-[var(--color-border)] rounded-lg shadow-[var(--shadow-medium)] z-50 p-3 min-w-[200px]"
    >
      ...
      <div className="space-y-2">
        <div className="flex items-center justify-between gap-3">
          ...
          <select
            ...
            className="flex-1 text-xs bg-[var(--color-cream)] border border-[var(--color-border)] rounded px-1.5 py-1 text-[var(--color-ink)] cursor-pointer focus:outline-none"
          >
            {Array.from({ length: 24 }, (_, i) => (
          ...
          <button
            ...
            className={`px-2 py-0.5 text-[9px] rounded border cursor-pointer transition-colors ${
              workStart === p.s && workEnd === p.e
                ...
                : 'bg-[var(--color-cream)] text-[var(--color-ink-muted)] border-[var(--color-border)] hover:border-[var(--color-border-dark)]'
            }`}
          >
            {p.label}
          </button>
